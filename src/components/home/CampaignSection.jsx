...
import { campaignBannerText, campaignCardText } from '../../i18n/artworkText.js'
...
function CampaignSection() {
  ...
  return (
    <section id="campaign" className="campaign-section" aria-labelledby="campaign-title">
      <Container>
        <div className="campaign-section__intro" aria-label={t("キャンペーン紹介スライド")}>
          ...
          */}
          ...
          <div className="campaign-section__intro-pagination" aria-label={t("キャンペーンスライド選択")}>
            {campaignBanners.map((banner, index) => (
              <button
                key={banner.image}
                type="button"
                aria-label={t(`${index + 1}枚目のキャンペーン画像を表示`)}
        ...
        <LocalizedImageCaption lines={campaignBannerText[activeBanner]} />
        <SectionTitle
          id="campaign-title"
          eyebrow={t("SPECIAL OFFER")}
          title={t("CAMPAIGN")}
          description={t("GYEOLDAMから、今だけの特別なご案内です。")}
          align="center"
        />
          ...
          {homeCampaigns.map((campaign, index) => (
            <article key={campaign.id} className="campaign-card">
              <div className="campaign-card__media">
                ...
                */}
                {campaign.image ? (
                  <img className="campaign-card__image" src={campaign.image} alt={t(campaign.imageAlt)} />
                ) : (
                  <ImagePlaceholder
                    label={t(`キャンペーン ${String(index + 1).padStart(2, '0')}`)}
                    aspectRatio="4 / 3"
                    ariaLabel={t(`${campaign.title} 画像準備中`)}
                  />
                )}
              </div>
              <div className="campaign-card__body">
                <LocalizedImageCaption lines={campaignCardText[index]} />
                <h3>{t(campaign.title)}</h3>
                <p>{t(campaign.description)}</p>
                <Link className="campaign-card__link" to="/treatment">{t("詳しく見る ")}<span aria-hidden="true">→</span>
                </Link>
              </div>
            </article>
          ))}
        ...
        <p className="campaign-section__closing-copy">{t("美しさは、日々の小さな瞬間から生まれます。")}<br />{t("自分らしさを大切にしながら、心地よく過ごせる時間を。")}<br />{t("一人ひとりに寄り添い、自然な美しさを引き出します。")}<br />{t("ゆっくりと流れる時間の中で、新しい自分に出会える場所へ。")}</p>
      </Container>
    </section>
  )
}
